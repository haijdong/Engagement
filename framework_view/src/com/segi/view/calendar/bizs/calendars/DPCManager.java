...
public final class DPCManager {
     ...
     */
    public void setDecorOnTime(List<String> date) {
    	setDecor(date, DECOR_CACHE_ON_TIME);
    }
     ...
     * @param date 日期列表 List of date
     */
    public void setDecorNotOnTime(List<String> date) {
    	setDecor(date, DECOR_CACHE_NOT_ON_TIME);
    }
...
//            }
    ...
    private DPInfo[][] buildDPInfo(int year, int month) {
        ...
        Set<String> decorOnTime = DECOR_CACHE_ON_TIME.get(year + ":" + month);
        Set<String> decorNotOnTime = DECOR_CACHE_NOT_ON_TIME.get(year + ":" + month);
        for (int i = 0; i < info.length; i++) {
            for (int j = 0; j < info[i].length; j++) {
                ...
                if (null != decorOnTime && decorOnTime.contains(tmp.strG)) tmp.isOnTime = true;
                if (null != decorNotOnTime && decorNotOnTime.contains(tmp.strG)) 
                	tmp.isNotOnTime = true;
                info[i][j] = tmp;
            }
        }
        return info;
