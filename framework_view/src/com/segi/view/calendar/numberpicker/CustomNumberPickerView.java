...
public class CustomNumberPickerView extends View{
    ...
    public interface OnValueChangeListener{
        void onValueChange(CustomNumberPickerView picker, int oldVal, int newVal);
    }

    public interface OnValueChangeListenerRelativeToRaw{
        void onValueChangeRelativeToRaw(CustomNumberPickerView picker, int oldPickedIndex, int newPickedIndex,
                                        String[] displayedValues);
    }

    public interface OnValueChangeListenerInScrolling{
        void onValueChangeInScrolling(CustomNumberPickerView picker, int oldVal, int newVal);
    }
    ...
    public interface OnScrollListener {
        ...
        void onScrollStateChange(CustomNumberPickerView view, int scrollState);
    }
    ...
    public CustomNumberPickerView(Context context, int styleId) {
    	super(context);
    	initAttrByStyle(context, styleId);
    	init(context);
    }
    ...
    public CustomNumberPickerView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        initAttr(context, attrs);
        init(context);
    }
    
    private void initAttrByStyle(Context context, int styleId)
    {
    	if (0 == styleId) {
			return;
		}
    	TypedArray a = context.obtainStyledAttributes(styleId, R.styleable.NumberPickerView);
    	searchAttrForSet(context, a);
    }
     ...
     * @param a
     */
    private void searchAttrForSet(Context context, TypedArray a)
    {
    	if (null != a) {
        ...
        if(currPickedIndex != mPrevPickedIndex && mRespondChangeOnDetach){
            try {
                if (mOnValueChangeListener != null) {
                    mOnValueChangeListener.onValueChange(CustomNumberPickerView.this, mPrevPickedIndex + mMinValue, currPickedIndex + mMinValue);
                }
                if (mOnValueChangeListenerRaw != null) {
                    mOnValueChangeListenerRaw.onValueChangeRelativeToRaw(CustomNumberPickerView.this, mPrevPickedIndex, currPickedIndex, mDisplayedValues);
                }
            }catch (Exception e){
                e.printStackTrace();
            }
    ...
    private void respondPickedValueChanged(int oldVal, int newVal, Object respondChange){
        ...
        if(oldVal != newVal){
            if(respondChange == null || !(respondChange instanceof Boolean) || (Boolean) respondChange) {
                if(mOnValueChangeListener != null) {
                    mOnValueChangeListener.onValueChange(CustomNumberPickerView.this, oldVal + mMinValue, newVal + mMinValue);
                }
                if(mOnValueChangeListenerRaw != null){
                    mOnValueChangeListenerRaw.onValueChangeRelativeToRaw(CustomNumberPickerView.this, oldVal, newVal, mDisplayedValues);
                }
            }
        }
    ...
    private TextUtils.TruncateAt getEllipsizeType() {
    	if(TEXT_ELLIPSIZE_START.equals(mTextEllipsize)){
    		return TextUtils.TruncateAt.START;
    	}else if(TEXT_ELLIPSIZE_MIDDLE.equals(mTextEllipsize)){
    		 return TextUtils.TruncateAt.MIDDLE;
    	}else if(TEXT_ELLIPSIZE_MIDDLE.equals(TEXT_ELLIPSIZE_END)){
