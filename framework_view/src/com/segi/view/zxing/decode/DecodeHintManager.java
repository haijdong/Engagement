...
package com.segi.view.zxing.decode;
...
final public class DecodeHintManager {
  ...
  public static Map<DecodeHintType,?> parseDecodeHints(Uri inputUri) {
    ...
    for (DecodeHintType hintType: DecodeHintType.values()) {
      ...
      if (hintType.getValueType().equals(int[].class)) {
        ...
        for (int i = 0; i < values.length; i++) {
          try {
        	...
        	Logger.w(TAG, "Skipping array of integers hint " + hintType + " due to invalid numeric value: '" + values[i] + '\'');
            array = null;
            break;
          }
        }
      ...
      Logger.w(TAG, "Unsupported hint type '" + hintType + "' of type " + hintType.getValueType());
    }

    Logger.i(TAG, "Hints from the URI: " + hints);
    return hints;
  }
      ...
      if (extras.containsKey(hintName)) {
        if (hintType.getValueType().equals(Void.class)) {
          ...
          if (hintType.getValueType().isInstance(hintData)) {
        	...
        	Logger.w(TAG, "Ignoring hint " + hintType + " because it is not assignable from " + hintData);
          }
        }
      }
    }

    Logger.i(TAG, "Hints from the Intent: " + hints);
    return hints;
  }
