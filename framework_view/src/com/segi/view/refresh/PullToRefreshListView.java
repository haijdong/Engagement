...
import com.segi.view.refresh.ILoadingLayout.State;
...
//        getRefreshableView().scrollBy(0, -1);
    }
...
//            return true;
//        }
        if (0 == mListView.getFirstVisiblePosition()) {
...
//        }
    ...
    /**
     * 判断当前ListView的最后一个item是否到底部，是则将ListView的TranscriptMode参数设置为TRANSCRIPT_MODE_NORMAL（当最后一个item可见并且是底部时，手动新增数据时(比如评论)自动滑动到底部）
     * 若当前页面可以上拉加载更多，在上拉加载时需将TranscriptMode参数设置为TRANSCRIPT_MODE_DISABLED（不可用）
     */
    public void setListViewTranscriptMode(){
        if (null != mListView){
